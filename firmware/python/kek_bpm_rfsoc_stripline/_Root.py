...
class Root(pr.Root):
    def __init__(self,
        ...
        self.ampFaultWithHdr = [bpmShared.PrependLocalTime() for i in range(4)]
        ...
        for i in range(4):
            ...
            self.ampFaultProc[i] = bpmShared.RingBufferProcessor(
        ...
        for i in range(4):
            ...
            self.ampFaultBuff[i] >> self.ampFaultWithHdr[i] >> self.dataWriter.getChannel(i+8)
        ...
        self.epics = pyrogue.protocols.epicsV4.EpicsPvServer(
            base      = 'kek_bpm_rfsoc_stripline_ioc',
            root      = self,
            pvMap     = None,
            incGroups = None,
            excGroups = None,
    ...
    def getFaultEventStatus(self):
        ...
        newData = True
        for i in range(4):
            if not self.ampFaultProc[i].Updated.get():
                newData = False
        ...
        if (self.EnableAutoReopen.get()) and newData:
            ...
            # Reset the flags
            [self.ampFaultProc[i].Updated.set(0) for i in range(4)]
    ...
    def start(self,**kwargs):
        ...
        bpmShared.Root.StartupInit(self)
